...
- 2026-09-25 사실감 조명([realistic-lighting](../src/lib/render/realistic-lighting.ts))으로 바꿨다.
  - 조명: 천장 중앙 SpotLight 하나(90° 원뿔·완전 penumbra로 평판 등처럼 cos 감쇠, 부드러운 PCF 그림자)와 렌더러마다 하나씩 만드는 절차적 욕실 환경맵(PMREM)을 쓴다. 환경맵에서 열린 앞면(카메라 쪽)이 밝아 뒷벽을 채운다. 반정밀도 색 타깃이 없으면 같은 밝기의 HemisphereLight로 대체한다.
...
- 2026-09-26 내보내기 사실감 1단계([결과](export-realism-stage-1-results-20260926.md))
  ...
  - **여러 장 겹쳐 찍기**([accumulate](../src/lib/room-viewer/accumulate.ts)): `export({ quality })`에서만 동작한다.
    ...
    - 누적 동안만 SpotLight 그림자를 focus 0.8·2048² 맵으로 바꾸고, 끝나면 값과 맵을 되돌린다. 실시간 그림자(원뿔 90° → 그림자 화각 180°라 드리우지 않음)는 그대로다.
    - 매 장 1픽셀을 읽어 GPU 완료를 기다린다. 진행률·시간 예산·취소가 실제 시간을 따른다.
    - 기본 32장, 예산 10초다. 첫 장 시간으로 장 수를 줄이고, 16장 미만이면 등을 100mm 안에서만 옮긴다(그림자 겹침 없이 PCF 디더링을 줄임).
