...
1. 메인 `/`와 `/materials`에서 로그인 없이 공용 자재를 구경하고, `/try`에서 빈 공간을 만들어 기본 자재를 검색·배치한다. 사진·AI·정식 저장·시안 비교·견적·출력 조작은 로그인 안내로 연결한다.
...
| 사진·AI·시안 비교·견적·출력 | 로그인 필요 | 본인 프로젝트 범위 | 본인 또는 허용된 관리자 프로젝트 범위 |
