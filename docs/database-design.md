...
이 문서는 아이디·(선택) Google 로그인, 회원·관리자 권한, 자재 라이브러리와 프로젝트 저장 구조를 설명한다. 실행용 SQL은 `migrations/d1/0001_auth.sql`부터 `0007_username_auth.sql`까지다. 기존 마이그레이션은 변경하지 않고 `0006`으로 계정별 진단 아카이브, `0007`로 아이디 로그인 컬럼을 추가한다. **2026-09-23 사용자가 원격 `sjn`에 0007의 `username`·`displayUsername` 컬럼을 직접 추가했고(auth meta version 1 유지), 같은 날 사용자 승인으로 Time Travel 북마크 기록 후 남은 고유 인덱스 `user_username_idx`와 `d1_migrations`의 0007 기록을 추가했다. 원격 목록은 `No migrations to apply`이며 회원 수는 변하지 않았다.**
...
1. 메인 `/`와 `/materials`에서 로그인 없이 공용 자재를 구경하고, `/try`에서 빈 공간을 만들어 기본 자재를 검색·배치한다. 사진·AI·정식 저장·출력 조작은 로그인 안내로 연결한다.
2. `/login`에서 **Google로 시작하기**를 누른다. 첫 로그인은 일반 회원 가입, 이후에는 재로그인이다. 체험에서 로그인을 선택했다면 같은 탭 초안을 이어 본인 D1/R2 프로젝트로 저장한다. 일반 회원은 `/`의 내 프로젝트에서 기존 자료를 열고 모든 회원 기능을 사용한다.
...
기본 개발은 `npm run dev`의 vinext/Workers + 격리된 로컬 D1/R2를 사용한다. 자재 조회를 위해 `npm run db:dev:migrate`가 필요하고, 계정 기능에는 `.dev.vars`와 Google callback 설정도 필요하다. 공개 메인·빈 공간 체험은 OAuth 준비와 분리한다. `/login`에서 설정 오류가 보이면 계정 준비 상태를 확인하며 기존 계정 작업을 익명 IndexedDB 편집으로 우회하지 않는다. 기존 브라우저 자료는 삭제·자동 업로드하지 않는다.
...
| 메인·공용 자재 목록·상세·검색·표시 이미지 | 현재 활성 공용 범위 | 현재 활성 공용 범위 | 현재 활성 공용 범위 |
| `/try` 빈 공간 생성·기본 자재 검색/배치 | 같은 탭 임시 초안 | 가능; 본인 프로젝트로 저장 | 가능; 본인 프로젝트로 저장 |
| 정식 프로젝트 생성·편집·복제·삭제 | 로그인 필요 | 본인 것만 | 본인 것만 |
| 체험 속성 조절·견적·시안 추가/비교 | 같은 탭의 임시 체험만 허용 | 본인 프로젝트 범위 | 본인 또는 허용된 관리자 프로젝트 범위 |
| 사진·AI·정식 저장·이미지 출력 | 로그인 필요 | 본인 프로젝트 범위 | 본인 또는 허용된 관리자 프로젝트 범위 |
...
| 같은 탭 sessionStorage | `/try` 임시 공간·배치·공개 DTO·빈 방 재생성 정보 | 새로고침·OAuth 왕복 복원용. 사진 바이트·계정 자료를 넣지 않으며 익명 D1/R2 쓰기 없음 |
| 진단 아카이브 D1/R2 | 진단 메타데이터와 JSON | 시작 계정에 고정, 계정별 최근 최대 20건·25MiB |

`GET /api/catalog/materials`는 로그인 없이 `scope='shared' AND active=1 AND purpose='catalog'`이며 `reconstruction` 정보가 없는 자재의 **현재 버전**만 반환한다. 목록 DTO에는 표시 이름·규격·속성·가격·공개 이미지 API URL만 포함하고, 회원 정보·소유자 ID·프로젝트 JSON·R2 내부 키·전체 원본 payload는 넣지 않는다. 이 공개 경로는 D1/R2만 검사하고 OAuth secret·세션 준비와 분리한다. `/api/d1/**`와 관리자·사진/AI API는 기존 활성 회원 검증을 유지한다.

`GET /api/catalog/placement`는 `{placements}` 목록, `?materialId=...`는 현재 자재의 배치 DTO 한 건을 반환한다. [PublicPlacement](../src/lib/catalog/placement-contract.ts)는 실제 material/version ID·규격·설치 방식·타일 줄눈·텍스처·제품 방향/기준점과 표시 이미지 metadata만 포함한다. `sourceAssetId`, `product3d`, 입력 사진·메시 참조·전체 payload·가격은 배치 DTO에 없다. 표시 이미지가 일부만 공개 가능하면 해당 자재를 배치 목록에서 제외하고 단건은 404로 처리하여 로그인 전후 뷰 인덱스가 바뀌지 않게 한다. 임의 과거 버전 조회는 제공하지 않는다.

`GET /api/catalog/images?id=...`는 로그인 없이 현재 공용 버전에서 직접 사용하는 표시 이미지인지 서버에서 다시 확인한다. 종류는 texture/product/preview, 형식은 PNG/JPEG/WebP로 제한하고 no-store·nosniff 응답을 유지한다. 개인 프로젝트 사진, 분석 원본, 제품 메시, `source_asset_id`를 따라간 원본은 공개하지 않는다. 버킷을 공개로 바꾸지 않는다. 자재 비활성화나 버전 교체 후 이전 이미지 URL도 공개 조건을 다시 통과해야 한다.

`/try`의 [게스트 세션](../src/lib/guest/session.ts)은 `sjn:guest-draft:v1`에 같은 탭 임시 초안을 보관하고 실제 이미지 바이트는 메모리에서 처리한다. 계정 IndexedDB나 기존 로컬 자료는 읽지 않으며 사진·AI·진단 아카이브·D1/R2 쓰기를 하지 않는다. Google 로그인 복귀 후에는 계정과 참조를 다시 확인하고 기존 회원 저장 API로 본인 프로젝트를 만든다. 서버 저장 확정 전 초안을 유지하고 재시도·계정 변경을 처리한다. 공개 버전이 바뀌거나 비활성화되면 체험 복원 시 자재가 자동으로 다른 버전으로 교체되지 않아야 한다.
...
명시적 Wrangler `--env`를 쓰는 경우 바인딩·변수·secret과 모든 적용/배포 명령의 환경을 일치시킨다. 기본 로컬 개발은 `npm run db:dev:migrate` 후 `npm run dev` / `npm run dev:vinext`로 3000 포트의 Workers와 격리된 D1/R2를 사용한다. 개발·운영 모두 계정 기능에는 Google 로그인이 필요하며 인증·DB 준비 실패 시 기존 계정 자료를 익명 편집으로 전환하지 않는다. 공개 메인·`/try`와 공용 GET 목록/이미지/배치 API는 로그인 준비와 분리하고 공용 자료 조회 실패만 해당 영역에 표시한다.
...
| 카탈로그 API | 익명 현재 공용 목록·배치·이미지 조회, OAuth 누락과 독립, 소유자/R2 키·원본·메시·product3d 비노출, 비활성·개인·과거 버전 거부 |
...
| 체험·복귀 | 같은 탭 새로고침·OAuth 취소/복귀, 초안 검증·용량 오류·자재 버전 변경 안내, 로그인 후 동일 초안 저장·재시도 중복 생성 방지 |
| 실패 처리 | R2 업로드 실패·D1 commit 실패·revision 충돌, 계정/관리자 세션 만료 시 잠금, 공개 체험과 계정 복구본 분리 |
...
npx vitest run tests/catalog-auth-boundary.test.ts tests/d1-catalog.test.ts tests/d1-storage.test.ts tests/d1-auth.test.ts
