...
## 실사진 8장 검증과 추가 보정 (2026-09-24, 2차)

사용자가 준 제품 사진 8장(320×320 JPG, 배경 있음)을 앱과 같은 Worker로 **배경 제거(BiRefNet FP16) → 입체화(TripoSR, WebGPU) → 전후 캡처**까지 실제로 실행했다(`tests/product3d-batch-browser.ts`). 모델은 로컬 파일만 썼다. 결과와 비교 이미지는 `test-results/product3d-batch/<이름>-comparison.png`, `summary.json`에 있다. 장치는 RTX 4070 Ti(WebGPU, Chrome 153 headless)다.

| 사진               | 배경 제거 | 입체화 처리 | 정점    | 자동 수평 | 사진 정면 밝기 | 조명 보정 정면·90° |
| ------------------ | --------- | ----------- | ------- | --------- | -------------- | ------------------ |
| 벽 선반(소품 포함) | 2.6초¹    | 10.6초      | 112,021 | 0°        | 184            | 186·191            |
| 일체형 비데 변기   | 1.4초     | 11.9초      | 286,978 | **25.0°** | 223            | 215·220            |
| 사각 욕조          | 1.3초     | 11.5초      | 172,840 | 21.8°     | 230            | 218·214            |
| 욕조(실내 사진)    | 1.4초     | 11.4초      | 256,162 | 8.3°      | 174            | 175·185            |
| 욕조(위에서 찍음)  | 2.5초¹    | 11.0초      | 183,720 | **30.6°** | 193            | 198·203            |
| 휴지걸이 스탠드    | 1.3초     | 10.4초      | 36,843  | 6.8°      | 135            | 158·161            |
| 스툴               | 1.4초     | 12.1초      | 394,056 | 0°        | 189            | 178·195            |
| 원형 거울          | 1.4초     | 13.8초      | 155,038 | 0°        | 206            | 200·183            |

¹ 그 실행의 첫 사진이라 모델 준비 시간이 포함됐다. 입체화 모델 초기화는 사진마다 2.4~3.2초가 따로 든다(새 Worker).

- CPU(wasm, 격자 160) 실추론: 스툴 **395초**(약 6.6분), 152,708 정점.
- 위에서 내려다본 사진(비데 변기, 위에서 찍은 욕조)은 자동 수평이 실제로 세웠다. 1차 fixture에서는 확인하지 못했던 효과다.

### 발견한 문제와 보정

1. **조명 보정이 너무 밝았다.** 흰 제품 스튜디오 사진에서 정면이 203–239로 원본(174–230)보다 밝아 음영이 하얗게 날아갔다. 광량을 방향광 2.4→1.8, 반구광 2.2→1.9로 낮춰 정면이 원본 정면과 비슷해졌다(비데 235→215, 사각 욕조 239→218). 어둡게 찍힌 1차 fixture 변기는 정면 166→151이 됐다(원본 정면 132).
2. **표면 망치 자국 무늬.** Taubin 스무딩은 격자 몇 칸 크기의 요철을 일부러 남겨, 조명 보정에서 표면 전체가 두드려 편 것처럼 보였다. 음영용 법선을 이웃 평균 40회로 고르게 하는 `shadingNormals`로 바꿨고(24회보다 40회가 뚜렷이 깨끗했다), 공간 둘러보기의 `lit` 각도도 같은 법선을 쓴다.
3. **어두운 유색 부품 얼룩.** 호박색 병이 흰색·갈색 얼룩으로 칠해졌다. 원인은 병의 어두운 쪽이 밝기 기준 미만이라 분류에서 빠지고, 반사광 점(무채색, 흰 재질)이 그 빈자리를 채운 것이다. 이제 자기 색도가 유색 재질과 뚜렷이 같은 어두운 꼭짓점은 그 재질로 보고, 이웃 다수결로 외딴 점을 정리한다. 카메라 쪽 어두운 부분을 검정 부품으로 두는 방법도 시험했지만 fixture 변기의 부품 사이 틈이 검은 균열로 나타나 되돌렸다.
4. **떨어져 복원된 부품이 지워졌다.** 휴지걸이의 롤과 가로대가 얇은 기둥 연결 없이 따로 복원됐는데, "가장 큰 덩어리만 남김"이 이를 지웠다. 전체 삼각형의 5% 이상인 조각은 남기도록 바꿨다(롤 약 19%). 2%로 두면 위에서 찍은 욕조 사진 가장자리의 쟁반(약 3%)이 남아 자동 수평이 30.6°→0°로 틀어져 5%로 정했다. fixture 사진의 휴지걸이(따로 놓인 물체)는 이제 남는다(149,819→161,081 정점).

단위 테스트를 추가했다: 먼 쪽 짙은 그림자, 유색 부품 위 반사광 점, 따로 떨어진 큰 부품 유지.

### 남는 한계 (8장에서 관찰)

- 배경 제거는 제품과 함께 놓인 소품(선반 위 병·크림, 휴지걸이 옆 병, 욕조 사진 아래 쟁반)도 남긴다. 5% 이상이면 입체로 함께 만들어진다. 소품이 없는 사진이 좋다.
- 얇은 크롬 기둥·가로대는 TripoSR이 가늘게 끊기거나 뾰족하게 복원한다. 거울·크롬의 반사, 욕조 안쪽 오목한 공간(사각 욕조는 속이 찬 상자로 복원), 스툴 좌판 가장자리의 들쭉날쭉함은 모델 한계다.
- 무채색 부품(검정 펌프, 거울 유리, 크롬)은 조명 보정에서 흰 재질로 합쳐진다. 소품의 작은 유색 부분(금색 병)도 전체의 3% 미만이면 합쳐진다.
- 320px 사진이라 입력(512)이 확대돼 세부가 흐리다.

### vinext 홈 버튼 버그 수정

`기본 공간으로 시작`이 vinext 개발 서버에서 눌러도 반응이 없던 원인을 확인했다. `/`는 공개 경로라 서버가 `GuestHome`을 먼저 그리고, 버튼이 활성 상태로 보이지만 React가 붙기(하이드레이션) 전이다. `goto('/')` 직후 버튼에는 React 속성이 없었고, 하이드레이션은 0.3~1.9초 뒤였다. 느린 회선의 실제 사용자도 같은 헛클릭을 겪을 수 있어, `GuestHome`의 버튼 5개를 하이드레이션 전까지 비활성으로 두었다(`useSyncExternalStore`). 수정 뒤 vinext에서 `material-images.spec.ts` 4개와 `여러 각도 자재: 공간에서…`가 통과했다.

### 테스트와 빌드

- `npx vitest run` **221개 파일 / 3,182개 통과**. `npm run typecheck`, 변경 파일 ESLint·Prettier 통과.
- `SJN_PRODUCT3D_REAL=1 npx playwright test --config playwright.product3d.config.ts` **17개 모두 통과**(7.5분, vinext, 실제 AI 포함, `material-images` 4개 포함).
  - 실제 추론 결과는 161,081 정점, 로컬 전송 1.59초, 모델 초기화 2.92초, 추론·메시 구성 11.33초였다.
  - 드래그 렌더는 200fps였다.
- 기본 config(Next 개발 서버)에서 게스트 홈 관련 e2e 39개를 실행했다(`guest-workspace`, `base-room`, `catalog-access`, `cloud-recovery`, `room-dimensions`). 36개 통과, 3개 실패다.
  - `base-room.spec.ts`의 `390px/320px 공간 설정과 편집기…` 2개는 좁은 화면에서 `공간 크기` 버튼이 `더보기` 메뉴 안에 있어 실패한다. 이 메뉴는 09-18 커밋(192019f)에서 들어왔고 테스트는 09-17 이후 갱신되지 않아, 이번 변경과 무관한 기존 문제다.
  - `guest-workspace.spec.ts`의 `속성 패널 슬라이더…`는 테스트 도우미가 fetch에서 `bad port`를 받은 일회성 오류였다. 단독 재실행에서 통과했다.
- `npm run build`(Next)와 `npm run build:vinext` 모두 통과했다. 오류는 없었다.

## 결과 품질 보정 검증 (2026-09-24, 1차)

위 2차 보정 전의 기록이다. 광량·법선·재질 분류·조각 정리는 2차에서 바뀌었다.

TripoSR 모델은 그대로 두고 조명 보정·자동 수평·형상 정리(256 격자, 가장 큰 덩어리, Taubin)·입력 가장자리 정리·사진 밖 각도 안내를 추가한 작업의 검증이다. 설명은 [제품 입체 편집 문서](./product3d-editor.md#결과-품질-보정)에 있다.

### 실제 추론 (`SJN_PRODUCT3D_REAL=1`)

- 장치: Windows, Chrome, NVIDIA RTX 4070 Ti 12GB, **WebGPU**. 모델 파일은 `tmp/multiview-model`에서 localhost로 스트리밍했다(다운로드 속도 측정값 아님).
- 새 파이프라인 결과: **149,819개 정점·299,636개 삼각형**(이전 160 격자 55,956 / 111,912). 메시 약 7.2MB로 25MB 한도 안이라 격자를 줄여 다시 추출하는 경로는 타지 않았다.
- 시간(2회 실행): 로컬 전송 1.90 / 1.92초, 모델 초기화 3.91 / 3.75초, 추론·메시 구성 **12.53 / 11.93초**(이전 160 격자 기록 8.63초). 격자 256의 밀도·색 질의가 늘어난 만큼 약 3.3~3.9초 길어졌다.
- 실제 AI PNG: 불투명 295,420픽셀, 완전 투명 751,375픽셀, 바깥 3px 제품 침범 0.
- CPU(wasm) 경로는 격자 160을 그대로 쓰므로 이번에 시간을 다시 재지 않았다.

### 전후 비교 (`node tests/run-browser-test.mjs tests/product3d-quality-browser.ts`)

같은 `ProductRenderer`로 정면·45°·90°를 캡처했다. 개선 전은 `원본 색`·수평 보정 없음, 개선 후는 `조명 보정`·자동 수평이다. 결과는 `test-results/product3d-quality/`(`*-comparison.png`, 각 PNG, `summary.json`).

| 메시                     | 정점 / 삼각형     | 표면 거칠기¹ | 제품 밝기 평균·편차 (정면 / 45° / 90°) |
| ------------------------ | ----------------- | ------------ | -------------------------------------- |
| 이전 파이프라인, 개선 전 | 55,956 / 111,912  | 0.396        | 132·40 / 105·53 / 64·41                |
| 이전 파이프라인, 개선 후 | 같음              | 같음         | 166·15 / 164·16 / 162·19               |
| 새 파이프라인, 개선 전   | 149,819 / 299,636 | **0.114**    | 134·38 / 105·50 / 71·42                |
| 새 파이프라인, 개선 후   | 같음              | 같음         | 165·16 / 164·16 / 163·18               |

¹ 정점과 이웃 평균 사이 거리 / 평균 모서리 길이. 격자 밀도와 무관하게 비교할 수 있다.

- 개선 전에는 돌릴수록 사진의 그림자 쪽이 보여 밝기가 132 → 64로 떨어졌다. 조명 보정 후에는 세 각도가 162~166으로 고르다.
- 새 메시는 요철이 약 71% 줄었다. 부피는 0.0801 → 0.0767(−4%)인데, 떠 있던 휴지걸이 조각이 빠진 영향이 포함된다.
- 조명 보정은 형상의 작은 요철을 드러낸다. 촘촘한 256 메시에서 뚜껑에 규칙적인 오목 무늬가 보여, 음영용 법선 스무딩 횟수를 정점 수에 비례(12~40회)하도록 고쳤다. 15만 정점에서 약 0.23초가 걸리며 조명 보정으로 처음 바꿀 때 한 번만 계산한다.
- 자동 수평은 두 메시 모두 보정 없음(identity)을 돌려줬다. 이 fixture는 쓰레기통이 붙어 복원돼 기준이 될 평평한 바닥·벽 면이 뚜렷하지 않고, 확실할 때만 적용하는 기준을 넘지 못했다. 기울어진 상자 점군을 1° 안으로 되돌리는 것은 단위 테스트로 확인했지만, 실제로 기울어진 복원 결과에서의 효과는 이번 fixture로 확인하지 못했다. 계산 시간은 22~29ms.
- 입력에 함께 찍힌 회색 쓰레기통은 TripoSR이 변기와 붙여 복원한다. 흰 도기와 밝기만 다른 무채색이라 조명 보정에서 같은 흰색 재질로 합쳐진다(알려진 한계).

### 테스트

- 단위: `npx vitest run` **221개 파일 / 3,179개 통과**. 새 테스트는 `product3d-albedo`, `product3d-upright`, `product3d-mesh-cleanup`이고, `product3d-pixels`(가장자리 정리), `product3d-pose`(`sourceViewAngle`), `product3d-apply`·`product3d-storage`(`shading` 기록·허용·거부), `product3d-geometry`(격자 축소), `room-viewer-fixtures`(`lit` 각도의 공간 표시)를 확장했다.
- `npm run typecheck`, 변경 파일 ESLint, Prettier 확인 통과.
- 브라우저: `SJN_PRODUCT3D_REAL=1 npx playwright test --config playwright.product3d.config.ts e2e/product3d.spec.ts` **13개 중 12개 통과**(6.8분, 실제 AI 포함). 새 테스트 `조명 보정·자동 수평·사진 밖 각도 안내·저장 모드 복원`은 두 모드의 PNG가 다른지, 자동 수평·실행 취소, 40° 초과 안내와 복귀, `lit` 저장과 새로고침 후 무추론 복원을 확인한다(`shading-lit.png`, `shading-baked.png`, `shading-reopened-ui.png`).
- 실패 1개 `여러 각도 자재: 공간에서 썸네일 변경…`은 홈의 `기본 공간으로 시작`을 눌러도 `공간 만들기` 창이 열리지 않아 멈췄다(2차에서 원인을 찾아 고쳤다). 이 설정의 vinext 개발 서버(`npm run dev`)에서만 재현되고 같은 설정의 `material-images.spec.ts` 3개도 같은 곳에서 멈췄다. 같은 테스트와 `material-images.spec.ts` 4개는 기본 설정(`npx playwright test`, Next 개발 서버)에서 모두 통과했다. 이번 변경 파일은 홈·공간 만들기 창을 건드리지 않는다. 변경 전 코드에서 같은 vinext 실패가 나는지는 확인하지 않았다.
- 모바일 터치 테스트는 캔버스 45% 높이를 누르던 지점이 하단 고정 저장 영역에 6px 걸려 실패했다. 새 도구 버튼을 숨겨도 캔버스 위치가 같아 이번 UI 변경과는 무관했고, 테스트가 저장 영역 위의 보이는 캔버스를 누르도록 고쳤다.
- 제품 PNG 다운로드가 0바이트로 저장되던 것은 spec의 `launchOptions`가 설정의 `downloadsPath`를 덮어써서였다. spec에 같은 경로를 넣어 고쳤다.
- 드래그 성능(1440×1100, 기존 fixture 메시): 실제 렌더 168회 **198fps**, 33.34ms 초과 0회.

### 확인하지 않은 범위

- CPU(wasm) 실추론 시간, 저사양 PC·실제 모바일 GPU.
- 사용자가 보여 준 세면대는 원본 투명 PNG가 없어 실행하지 않았다.
- 목재 하부장처럼 색이 다른 재질이 섞인 실제 제품 사진의 조명 보정은 합성 단위 테스트로만 확인했다.
- Next/vinext 프로덕션 빌드와 배포는 실행하지 않았다.
