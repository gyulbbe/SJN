...
현재 앱은 **개발·운영 모두 Google 로그인 + D1/R2**를 사용한다. 회원은 본인 프로젝트와 공용 자재를 이용하고, 관리자는 회원 관리·공용 자재 관리·다른 회원 프로젝트 조회/편집을 한다. 준비 실패나 세션 만료를 익명 IndexedDB 편집으로 우회하지 않는다. [DB 설계](database-design.md)에 권한·ERD·전체 DDL·기본 데이터가 있다.
...
브라우저에는 D1/R2 관리 키를 전달하지 않는다. 일반 API는 본인 자료만 다루고 관리자 전용 API는 현재 관리자와 해당 프로젝트 범위를 검사한다. 소유자와 관리자 행위자를 분리해 기록한다. 공용 자재 목록·이미지도 활성 로그인 세션이 필요하며 버킷 공개 설정은 필요 없다.

## 저장소와 접근을 선택하는 규칙

| 실행/설정 | 결과 |
| --- | --- |
| 기본 dev / dev:vinext | 개발 전용 D1/R2 + Google 로그인 |
| Workers + STORAGE_MODE=auto 또는 d1 + 준비 성공 | D1/R2 + 로그인; APP_ENV 이름으로 익명 편집을 열지 않음 |
| STORAGE_MODE=local 또는 supabase | 현재 선택 경로에서는 설정 오류·접근 차단 |
| 바인딩·스키마·secret 누락, 연결 실패·초기 timeout | 오류·재시도 안내, 익명 로컬 전환 없음 |
| 세션 만료·정지 | 편집기를 숨기고 로그인 안내; 본인 계정 IndexedDB 복구본 보존 |
...
비로그인 `/materials`·이미지·프로젝트 요청이 차단되고 `/login`이 안내되는지 확인한 다음, 로그인 후 공용 자재·개인 프로젝트 생성·새로고침·이미지 업로드·로그아웃을 확인한다. 두 계정과 두 브라우저로 소유권을 확인하고, 타 계정의 프로젝트/파일 URL을 직접 호출해 차단되는지도 검증한다. 원격 sjn과 로컬 개발 DB의 0001~0006 적용·초기 데이터·SQL 무결성 검증은 완료했다. 앱의 운영 Worker 연결·실제 Google 로그인·R2 접근과 Workers CPU 제한 검증은 미실행이다.
