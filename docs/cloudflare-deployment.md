...
현재 앱은 개발·운영 모두 메인·공용 자재 구경·`/try` 빈 공간 체험을 로그인 없이 제공한다. 계정 프로젝트·사진/AI·정식 저장·출력에는 Google 로그인과 D1/R2가 필요하다. 체험은 같은 탭 `sessionStorage`에만 임시 보관하고 로그인 후 본인 프로젝트로 저장한다. 인증·저장소 준비 실패나 세션 만료로 기존 계정 자료를 익명 작업 공간으로 전환하지 않는다. 설정은 [D1 + R2 + Google 로그인 설정](cloudflare-storage-setup.md)을 따른다.
