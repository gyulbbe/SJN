...
제품 방향 사진을 실제 AI 모델로 처리해 원본·결과·시간을 확인하고, 원하는 결과를 제품 사진으로 적용할 수 있다. AI 실행만으로 자재를 변경하지 않는다. **투명 PNG 업로드**를 눌렀을 때 선택한 방향의 편집 중 이미지만 교체하며, 기존 원본과 저장된 자재 버전은 보존한다.

## 사용법

1. 자재 관리의 **자재 등록 / 자재 수정**, 또는 편집기의 **신규 자재 등록**을 연다.
2. 타일이 아닌 제품 카테고리를 선택하고 **제품 방향 이미지 올리기**로 사진을 올린다.
3. 각 사진 미리보기의 **AI 배경 제거 테스트**를 누른다. 누른 사진만 처리한다.
...
**투명 PNG 다운로드** 옆 **투명 PNG 업로드**를 누르면 파일 선택 없이 현재 결과를 선택한 방향 사진에 적용하고 결과 창을 닫는다. 원본 해상도와 투명도, 촬영 방향·배치 기준점은 유지한다. 다른 방향과 대표 이미지는 바꾸지 않는다. 이후 **자재 등록 / 새 버전 저장**을 눌러야 자재에 최종 저장된다.

모델 준비·추론·이미지 적용 중에는 중복 실행을 막는다. 적용 중에는 닫기와 Escape를 막으며, 저장 실패 시 현재 결과와 기존 제품 사진을 유지해 업로드를 재시도할 수 있다. 제품의 수동 배경 지우기 기능은 제거했다. 타일 네 점 정면 보정과 과거 수동 편집 자산의 읽기 호환은 유지한다.
...
- 결과 표시·배경색·확대·다운로드는 메모리의 결과를 사용한다. **투명 PNG 업로드**만 선택한 저장소에 새 제품 자산을 저장한 후 편집 중 방향 사진을 교체한다. 로컬 모드에서는 IndexedDB에만 저장한다. 원본 참조와 `ai-alpha` 메타데이터를 보존해 재실행 시 적용한 투명 사진을 입력으로 사용한다. 자재 버전 갱신은 자재 저장 시 수행하며, 서버 모드 연결·배포는 별도 검증 대상이다.
...
일반 UI 확인에는 모델 다운로드를 요구하지 않는다. 실추론 테스트는 공개 실사 사진을 준비하고 환경변수로 명시적으로 실행한다. 실추론 테스트는 결과 PNG를 대체하거나 모델 응답을 모킹하지 않는다. `background-removal-apply.spec.ts`는 적용·저장·오류 UI만 분리 검증하도록 Worker 결과를 모의하며, AI 품질 검증으로 계산하지 않는다.
...
## 투명 PNG 적용 및 수동 도구 제거 검증 (2026-09-10)

이 절은 위의 초기 테스트·캐시 변경 검증 이후 추가된 기능에 대한 최신 결과다.

- 제품 수동 배경 지우기 UI와 브러시 코드를 제거하고 타일 네 점 정면 보정은 유지했다.
- 결과 창의 **투명 PNG 다운로드 → 투명 PNG 업로드** 버튼 배치를 스크린샷으로 확인했다. 업로드는 파일 선택 없이 현재 결과를 원본 해상도의 새 제품 자산으로 저장하고 선택한 방향만 교체한다.
- `e2e/background-removal-apply.spec.ts`: **모의 Worker 결과 기반 UI 3개 통과**. 다른 방향·대표 이미지·기준점·기존 버전 보존, 적용 전/중 중복 방지, Escape 차단, 저장 실패 후 같은 결과 재시도, 적용 없이 닫기, 저장·새로고침 및 적용된 이미지로 재추론 입력을 확인했다. AI 추론 품질 검증과 구분한다.
- 버튼·네트워크 실패·Worker 미지원·취소·모바일을 포함한 Next.js 브라우저 회귀 **7개 통과 (11.1초)**. 별도 실추론 opt-in 1개는 이 회귀 실행에서 제외하고 아래에서 실제 실행했다.
- **실제 BiRefNet 4사진 통합 테스트 1개 통과 (1.6분)**. 흰색 변기·램프·정원 의자·기존 알파 사진의 PNG 다운로드, 배경색 변경과 확대를 확인하고 마지막 실제 결과를 적용 → 자재 등록 → 새로고침 → 재진입해 투명도 보존을 확인했다. 모델 다운로드 42.44초, 이미지 처리 3.05~3.33초. 후속 3회 모델 요청 0건, 외부 이미지 업로드 0건. 기존 고성능 검증 PC 결과로 저사양 성능을 의미하지 않는다.
- Cloudflare **로컬 Wrangler**에서 새 적용 UI 3개 통과 (7.5초). Windows에서 기본 8787 포트 바인딩 오류가 발생해 테스트용 18787 포트로 실행했다. 배포 설정의 포트는 변경하지 않았다.
- Vitest **43파일 / 420개**, `npm run test:images`, lint, 타입 검사, Next.js 빌드, vinext/Cloudflare 빌드 통과.
- Supabase 실제 연결 및 외부 배포는 실행하지 않았다. 서버 자산 검증 스키마에는 `ai-alpha`를 추가했다.

실추론 PNG·화면·측정 JSON: `test-results/background-removal-apply-real/`.
검사 로그: `test-results/background-apply-unit.log`, `background-apply-lint.log`, `background-apply-types.log`, `background-apply-next-build.log`, `background-apply-vinext-build.log`, `background-apply-cloudflare-alt.log`.
