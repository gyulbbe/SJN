  ...
  await page.route('**/api/d1/assets?*', (route) =>
    route.fulfill({
      json: {
        ...
        url: '/cloud-recovery-fixture.png',
      },
    }),
  );
  await page.route('**/cloud-recovery-fixture.png', (route) =>
...
test('expired session keeps the editor and recovery in the same cloud account until explicit retry', async ({
  page,
}) => {
  const fixture = await cloudFixture(page);
  fixture.failure = 401;
  ...
  await expect(page.getByTestId('save-status')).toContainText('저장 실패');
  await page.getByLabel('프로젝트명', { exact: true }).fill('만료 상태에서 추가한 변경');
  await expect(page.getByTestId('recovery-status')).toHaveText('이 기기에 복구본 저장됨');
  await expect(page.getByTestId('save-status')).toContainText('저장 실패');
  expect((await drafts(page))[0].document.name).toBe('만료 상태에서 추가한 변경');
  expect((await drafts(page))[0].document.ownerId).toBe(fixture.server.ownerId);
  expect(fixture.saves).toHaveLength(1);
  fixture.failure = 0;
  await page.getByRole('button', { name: '저장 다시 시도', exact: true }).click();
  await expect(page.getByTestId('save-status')).toHaveText('클라우드에 저장됨');
  expect(fixture.server.name).toBe('만료 상태에서 추가한 변경');
});
...
test('local SPA back navigation preserves the existing save-on-leave behavior before 500ms', async ({
  page,
}) => {
  await page.route('**/api/storage/status', (route) =>
    route.fulfill({
  ...
  await page.getByLabel('프로젝트명', { exact: true }).fill('로컬 뒤로 이동 직전 편집');
  await page.goBack();
  await expect
    .poll(async () =>
      page.evaluate(async (projectId) => {
        ...
        try {
          return await new Promise<string>((resolve, reject) => {
            const request = database.transaction('projects').objectStore('projects').get(projectId);
