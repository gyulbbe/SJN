...
import { readImageHeader } from '../src/lib/images';
...
const inputHeader = readImageHeader(inputBytes);
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (p === '/input') {
      res.setHeader('Content-Type', inputHeader.mime);
      res.end(inputBytes);
      return;
    }
...
try {
  ...
  const result = await page.evaluate(
    async ({ base, captureAnalysis }) => {
      ...
      const heapSamples = [pageHeap()].filter((value): value is number => value !== undefined);
      const heapTimer = setInterval(() => {
        const value = pageHeap();
        if (value !== undefined) heapSamples.push(value);
      }, 100);
      const stages: string[] = [];
      const project = await m.createReconstructionProject(
        new File(
          [blob],
          'reference.' + (blob.type === 'image/png' ? 'png' : blob.type === 'image/webp' ? 'webp' : 'jpg'),
          { type: blob.type },
        ),
        m.DEFAULT_ROOM,
      ...
      const heapEnd = pageHeap();
      if (heapEnd !== undefined) heapSamples.push(heapEnd);
      ...
      await engine.setSnapshot(
        {
          scene: ((value) => value.designs.find((item) => item.id === value.activeDesignId)!)(saved).scene,
          beforeScene: saved.shared.comparison!.before,
          materials,
        },
        (id) => repos.assets.get(id),
      );
      ...
      const originalRgba = captureAnalysis ? Array.from(rgba.data) : undefined;
      ...
      const measurement = {
        ...
        renderer:
          gl && extension ? (gl.getParameter(extension.UNMASKED_RENDERER_WEBGL) as string) : 'unavailable',
        segmentation: 'DeepLab ADE20K / TFJS WASM / single thread',
        inputPixels: [masks.width, masks.height],
        pageJsHeapPeakBytes: heapSamples.length ? Math.max(...heapSamples) : null,
        pageJsHeapEndBytes: heapEnd ?? null,
        memoryScope:
          'Chromium main-page JS heap only; excludes segmentation Worker, WASM, GPU and total process RAM',
        phase: 'photo import + segmentation passes + baseline/model asset generation',
      };
      engine.dispose();
      return {
        creationMs,
        ...
        previews,
        materials,
        analysisInput: captureAnalysis
          ? {
              width: masks.width,
              height: masks.height,
              rgba: originalRgba!,
              wall: Array.from(masks.wall),
              floor: Array.from(masks.floor),
            }
          : undefined,
      };
    },
    { base, captureAnalysis: process.env.RECONSTRUCTION_CAPTURE_ANALYSIS === '1' },
  );
  for (const [name, data] of Object.entries(result.previews))
    await writeFile(output + '/' + name + '.png', Buffer.from(data.split(',')[1], 'base64'));
  if (result.analysisInput)
    await writeFile(output + '/analysis-input.json', JSON.stringify(result.analysisInput));
  await writeFile(
    output + '/result.json',
    JSON.stringify({ ...result, previews: undefined, analysisInput: undefined, errors, external }, null, 2),
  );
  console.log(
    JSON.stringify(
      {
  ...
  if (process.env.RECONSTRUCTION_EXPECT === 'mint-bathroom') {
    ...
    assert.ok(
      before.fixtures
        ...
        .every((f) => f.reconstruction?.version === 2 && !f.reconstruction.appearanceAssetId),
      'Standard mirrors and windows without source reflection crops',
    );
