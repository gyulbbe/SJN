...
const tile = await sharp(
  Buffer.from(
    '<svg xmlns="http://www.w3.org/2000/svg" width="800" height="500"><rect width="800" height="500" fill="#cbbfae"/><path d="M0 80 Q180 220 350 100 T800 150 M0 310 Q220 150 480 350 T800 300" stroke="#a99b89" stroke-width="4" fill="none"/><rect x="35" y="25" width="730" height="450" stroke="#f2ebdd" stroke-width="12" fill="none"/></svg>',
  ),
)
  .png()
  .toBuffer();
const fixture = await sharp(
  Buffer.from(
    '<svg xmlns="http://www.w3.org/2000/svg" width="600" height="700"><ellipse cx="300" cy="230" rx="180" ry="95" fill="#4d8a72"/><rect x="175" y="230" width="250" height="330" rx="25" fill="#75aa8b"/><ellipse cx="300" cy="230" rx="140" ry="60" fill="#c9ded0"/></svg>',
  ),
)
  .png()
  .toBuffer();
const jpeg = await sharp({ create: { width: 120, height: 80, channels: 3, background: '#407e64' } })
  .jpeg()
  .withMetadata({ orientation: 6 })
  .toBuffer();
const bundle = await build({
  stdin: { contents: "export {importImage} from './src/lib/images';", resolveDir: process.cwd() },
  bundle: true,
  write: false,
  format: 'iife',
...
try {
  ...
  const orientation = await page.evaluate(
    async (bytes) => {
      const { importImage } = (window as unknown as { ImageTest: typeof import('../src/lib/images') })
        .ImageTest;
      const input = new File([new Uint8Array(bytes)], 'oriented.jpg', { type: 'image/jpeg' });
      const saved: import('../src/lib/types').AssetRecord[] = [];
      const result = await importImage(input, 'original', {
        async put(asset) {
          saved.push(asset);
        },
        async get() {
      ...
      const sameOriginal = new Uint8Array(await result.original.blob.arrayBuffer()).every(
        (value, i) => value === bytes[i],
      );
      return {
        dimensions: [
        ...
        count: saved.length,
        sameOriginal,
        sourceLinked: result.preview.sourceAssetId === result.original.id,
      };
    },
    [...jpeg],
  );
  assert.deepEqual(orientation, {
    dimensions: [80, 120, 80, 120],
    count: 2,
    sameOriginal: true,
    sourceLinked: true,
  });
  ...
  await form
    ...
    .setInputFiles({ name: 'tile.png', mimeType: 'image/png', buffer: tile });
  ...
  assert.equal(
    await form.getByRole('button', { name: '정면 사진 AI 배경 제거 테스트', exact: true }).isEnabled(),
    true,
  );
  await form.getByRole('button', { name: '자재 등록', exact: true }).click();
  await form.waitFor({ state: 'detached' });
  ...
  const persisted = await page.evaluate(async () => {
    ...
    const assets = (await read<import('../src/lib/types').AssetRecord>('assets')).filter(
      (asset): asset is import('../src/lib/types').ImageAssetRecord => asset.kind !== 'product-mesh',
    );
    const tileVersion = versions
      .filter((value) => value.name === '테스트 직사각 타일')
    ...
    const alphaAt = async (blob: Blob) => {
      ...
      bitmap.close();
      return ctx.getImageData(Math.floor(canvas.width * 0.1), Math.floor(canvas.height * 0.1), 1, 1).data[3];
    };
    return {
      tileVersions: versions.filter((value) => value.name === '테스트 직사각 타일').length,
      tileSize: [tileAsset.width, tileAsset.height],
      uploadedAlpha: await alphaAt(fixtureAsset.blob),
      originalAlpha: await alphaAt(original.blob),
    };
  });
  ...
  console.log(
    JSON.stringify(
      {
        passed: [
          'EXIF 6 orientation and original bytes',
          'tile upload and four-point correction',
          'immutable material v2',
          'transparent product PNG upload and AI test entry',
          'original alpha preservation',
          'refresh restoration',
          'second-tab write lock',
        ],
        persisted,
        screenshot: 'test-results/material-library.png',
      },
      null,
      2,
    ),
