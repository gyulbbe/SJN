/** Real local DeepLab geometry evidence capture. Photos and binary masks stay under test-results. */
...
const manifestPath = process.env.GEOMETRY_OBSERVATION_MANIFEST;
if (!manifestPath) throw Error('Set GEOMETRY_OBSERVATION_MANIFEST to the private case list.');
...
const output = resolve(
  process.env.GEOMETRY_OBSERVATION_OUTPUT ?? 'test-results/reconstruction-floor-geometry-20260913/semantic',
);
  ...
  for (const item of entries) {
    ...
    try {
        ...
        async ({ origin, id }) => {
          ...
          try {
            const masks = await api.segmentRoom(
              ...
              { quality: 'reconstruction', signal: controller.signal },
            );
      ...
      const report = {
        ...
        external,
        scope:
          'Actual local DeepLab inference and current review; no Qwen, no calibration or physical-depth assertion.',
      };
