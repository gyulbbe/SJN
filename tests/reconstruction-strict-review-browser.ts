...
const output = 'test-results/reconstruction-strict-placement-20260914/review-ui';
...
try {
  ...
  await toiletCard.getByLabel('보류 후보 가로 위치 (%)', { exact: true }).fill('50');
  await toiletCard.getByLabel('보류 후보 세로·깊이 위치 (%)', { exact: true }).fill('50');
  await toiletCard.getByRole('button', { name: '모형 추가', exact: true }).click();
  ...
  await basinCard.getByLabel('보류 후보 가로 위치 (%)', { exact: true }).fill('37');
  await basinCard.getByLabel('보류 후보 폭 (mm)', { exact: true }).fill('620');
  await basinCard.getByLabel('보류 후보 설치 높이 (mm)', { exact: true }).fill('680');
  await basinCard.getByRole('button', { name: '모형 추가', exact: true }).click();
  await expect.poll(async () => before(await inspect()).fixtures.length).toBe(2);
  ...
  assert.equal(placedBasin.roomPlacement!.u, 0.37);
  assert.equal(placedBasin.reconstruction!.widthMm, 620);
  assert.equal(placedBasin.reconstruction!.baseHeightMm, 680);
  ...
  const verification = {
    ...
    validActorInputs: {
      toilet: { u: 0.5, v: 0.5, dimensions: 'unchanged' },
      basin: { kind: 'vanity then basin', basinVariant: 'wall', u: 0.37, widthMm: 620, baseHeightMm: 680 },
    },
