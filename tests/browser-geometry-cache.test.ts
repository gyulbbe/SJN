...
vi.mock('../src/lib/reconstruction/moge-browser/client', () => ({
  MogeBrowserClient: class {
    run = mocks.run;
...
function result(): MogeBrowserResult {
  return {
    ...
    dense: {
      width: 400,
      height: 300,
      intrinsics: { fx: 0.75, fy: 1, cx: 0.5, cy: 0.5 },
      diagnostics: { revision: MOGE_POSTPROCESS_VERSION },
    },
    ...
    timings: {
      ...
      inferenceMs: 100,
      postprocessingMs: 12,
      planeExtractionMs: 25,
      totalMs: 155,
    },
  } as unknown as MogeBrowserResult;
}
const run = (mask = segmentation(), signal = new AbortController().signal) =>
  bridge.analyzeGeometryInBrowser(photo(), mask, understanding(), signal);
beforeEach(async () => {
  vi.resetModules();
  vi.clearAllMocks();
...
describe('browser geometry bridge (authored Worker results, real cache; no inference quality claim)', () => {
  ...
  it.each([
    (value: MogeBrowserResult) => {
      value.metadata.sourceWidth = 401;
    },
    (value: MogeBrowserResult) => {
      value.metadata.inputHeight = 299;
    },
    (value: MogeBrowserResult) => {
      value.metadata.artifact = { ...MOGE_ARTIFACT, sha256: 'wrong' } as unknown as typeof MOGE_ARTIFACT;
    },
    (value: MogeBrowserResult) => {
      value.planes!.evidence.revision = 'old-plane-algorithm';
    },
  ])('rejects wrong Worker model/photo/processing identity and does not cache it', async (mutate) => {
    const wrong = result();
    mutate(wrong);
    mocks.run.mockResolvedValueOnce(wrong);
  ...
  it('reuses a persisted result across module reloads but never a corrupted same-photo result', async () => {
    ...
    const key = (await db.getAllKeys('results'))[0];
    const record = await db.get('results', key);
    record.analysis.observation.intrinsics.fy = 3;
    await db.put('results', record, key);
    db.close();
    vi.resetModules();
    bridge = await import('../src/lib/reconstruction/geometry-browser-client');
...
describe('administrator transient browser geometry', () => {
  it('does not read or save another owner geometry in either cache tier', async () => {
    ...
    expect((await run(segmentation(), signal)).measurement.cacheHit).toBe(false);
