...
function cancelled(signal?: AbortSignal | null) {
  signal?.throwIfAborted();
}
async function database() {
  return openDB('sjn-reconstruction-cloud-stages', 1, {
    upgrade(db) {
      db.createObjectStore('stages', { keyPath: 'key' }).createIndex('savedAt', 'savedAt');
    },
  });
}
async function read(key: string): Promise<Cached | undefined> {
  try {
    ...
    try {
      return (await db.get('stages', key)) ?? memory.get(key);
    } finally {
      db.close();
    }
  } catch {
  ...
  try {
    ...
    try {
      ...
      for (
        ...
        cursor && excess > 0;
        cursor = await cursor.continue(), excess--
      )
        await cursor.delete();
      await tx.done;
    } finally {
...
async function requestKey(form: FormData, scope: string) {
  ...
  for (const [key, value] of form.entries())
    parts.push([
      ...
      typeof value === 'string'
        ? value
        : `${value.type}:${value.size}:${await sha(await value.arrayBuffer())}`,
    ]);
  ...
  return sha(
    new TextEncoder().encode(
      JSON.stringify({
        cacheVersion,
        operation: cloudOperationCacheIdentity(form.get('operation')),
        scope,
        parts,
      }),
    ).buffer,
...
function wait(ms: number, signal: AbortSignal) {
  return new Promise<void>((resolve, reject) => {
    ...
    const abort = () => {
      clearTimeout(timer);
      reject(signal.reason ?? new DOMException('분석을 취소했어요.', 'AbortError'));
    };
...
async function execute(form: FormData, key: string, signal: AbortSignal, cacheAllowed = true) {
  const cached = cacheAllowed ? await read(key) : undefined;
  cancelled(signal);
  if (
    cached &&
    cached.key === key &&
    ...
    cached.checksum ===
      (await sha(new TextEncoder().encode(JSON.stringify({ key, value: cached.value })).buffer)) &&
    validAnalysisResponse(cached.value, 'cloudflare-workers-ai')
  ) {
    const value = structuredClone(cached.value);
    value.measurement = {
      ...(value.measurement as object),
      cacheHit: true,
      requestMs: 0,
      inferenceCalls: 0,
      httpAttempts: 0,
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    if (!response.ok) {
      ...
      throw Object.assign(
        new Error(typeof body.error === 'string' ? body.error : '설비 분석 요청에 실패했어요.'),
        {
          status: response.status,
          code: body.code,
          retryAfter: response.headers.get('Retry-After'),
          diagnostics: { ...diagnostics, httpAttempts: attempt + 1, inferenceCalls, unknownAttempts },
        },
      );
    }
    if (!validAnalysisResponse(body, 'cloudflare-workers-ai'))
      throw new Error('설비 분석 공급자 정보를 확인할 수 없어요.');
    body.measurement = {
      ...(body.measurement as object),
      cacheHit: false,
    ...
    const persisted =
      ...
      (await save({
        ...
        savedAt: Date.now(),
        value: body,
        checksum: await sha(new TextEncoder().encode(JSON.stringify({ key, value: body })).buffer),
      }));
    cancelled(signal);
    ...
    return persisted
      ...
      : {
          ...
          measurement: {
            ...(body.measurement as object),
            cacheWarning: '저장 공간이 부족해 완료 단계는 이 탭에서만 재사용해요.',
          },
        };
  }
}

async function subscribe(key: string, form: FormData, signal?: AbortSignal | null, cacheAllowed = true) {
  cancelled(signal);
  ...
  return new Promise<Record<string, unknown>>((resolve, reject) => {
    ...
    const abort = () =>
      finish(() => reject(signal?.reason ?? new DOMException('분석을 취소했어요.', 'AbortError')));
    signal?.addEventListener('abort', abort, { once: true });
...
export async function fetchAnalysis(provider: AnalysisProvider, init: RequestInit): Promise<Response> {
  if (provider !== 'cloudflare-workers-ai')
    throw new Error('기존 로컬 AI 분석은 종료됐어요. Gemma 분석을 선택해 주세요.');
  ...
  const cacheAllowed = analysisCacheAllowed(init.signal);
  // Transient runs never join ordinary flights or reuse another run's photo observations.
  const key = cacheAllowed
    ? await requestKey(init.body, available.cacheScope)
    : 'transient:' + crypto.randomUUID();
  const result = await subscribe(key, init.body, init.signal, cacheAllowed);
