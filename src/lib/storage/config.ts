export type StorageMode = 'local' | 'd1' | 'supabase';
...
export function configuredStorage(settings: RuntimeSettings): StorageStatus {
  if (settings.APP_ENV !== 'production')
    return { mode: 'local', ready: true, reason: 'local_environment', authRequired: false };
  ...
  if (!['auto', 'd1', 'supabase'].includes(selected)) return blockedStatus('invalid_configuration');
  return {
    mode: selected === 'supabase' ? 'supabase' : 'd1',
    ready: false,
    reason: 'ready',
    authRequired: true,
  };
...
export const STORAGE_MESSAGES: Record<StorageReason, string> = {
  ...
  local_selected: '',
  ready: '',
  user_selected_local: '로컬 작업 공간을 사용하고 있어요. 자료는 자동 업로드되지 않아요.',
...
export function localStatus(reason: StorageReason): StorageStatus {
  return { mode: 'local', ready: true, reason, authRequired: false };
}
