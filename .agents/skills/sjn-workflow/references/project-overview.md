...
공개 자재 화면은 관리자 분류 폼과 별도 스타일을 사용한다. 검색·종류, 사이즈·색상·표면 선택 칩, 결과 개수와 조건 초기화, 불러오는 중·등록 없음·검색 결과 없음·오류를 구분한다. 이미지 실패에도 카드 공간을 유지하고 자재 상세는 키보드로 닫은 뒤 열었던 카드에 초점을 돌려준다.
...
- 공개 자재 목록과 편집기 자재 패널은 현재 제품 종류·탭의 자재에서 사이즈(타일은 가로×세로, 제품은 0보다 큰 가로×세로×깊이)·색상·표면(마감) 칩을 만든다. 같은 항목에서는 하나라도, 항목끼리는 모두 일치하는 자재를 표시하고 현재 범위에 없는 선택은 무시한다. 서버 API는 바꾸지 않고 받은 자재 값으로 브라우저에서 거른다.
