...
| `/login` | 로그인 / 회원가입·Google로 계속하기; 취소·실패·정지·재시도 상태를 필요한 때만 안내하며 로그인된 회원은 내 프로젝트로 이동하고 체험 복귀는 `/try`에서 저장을 이어감 |
