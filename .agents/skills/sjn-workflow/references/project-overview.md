...
개발·운영 모두 메인과 공용 자재 목록·상세·검색을 로그인 없이 볼 수 있다. `/try`에서는 빈 공간을 만들고 기본 자재를 검색·배치한다. 사진·AI·정식 저장·시안 비교·견적·출력은 로그인이 필요하다. 일반 회원은 본인 프로젝트를 생성·편집하고 공용 자재를 사용한다. 관리자는 공용 자재·기준 분류·회원 역할/상태를 관리하며, 전용 화면에서 다른 회원의 프로젝트를 검색·조회·편집할 수 있다. 소유권은 유지하고 관리자 행위를 감사 기록에 남긴다. 기존 IndexedDB 자료와 계정별 복구본은 보존하고 게스트 체험과 섞지 않는다.
...
- 체험 초안은 `sessionStorage`에 같은 탭 범위로 임시 보관하여 새로고침·Google 로그인 이동 뒤 이어간다. 탭을 닫으면 사라질 수 있으며 서버 저장 완료를 의미하지 않는다. 로그인 성공 후 본인 D1/R2 프로젝트로 저장하고 실패하면 초안을 유지한다. 체험에서는 시안 하나의 기본 배치를 편집한다. 사진·AI·정식 저장·시안 비교·견적·출력과 타일 시공 세부 설정·밝기/색감·그림자·겹침 순서·새 배치 잠금은 로그인 안내를 표시한다. 기존 체험 초안의 배치 잠금 해제는 허용한다.
...
- 공개 자재 API는 현재 활성 공용 버전의 표시·배치 DTO와 직접 표시하는 이미지만 반환한다. 원본 사진·개인 자재·메시·입체화 원본 참조·전체 자재 payload는 공개하지 않는다. 배치 DTO에는 실제 버전 ID와 규격·줄눈·방향·기준점을 유지하여 로그인 후 동일 자재로 저장할 수 있다.
