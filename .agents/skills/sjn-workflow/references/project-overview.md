...
- 공간 둘러보기의 90° 회전·확대·이동과 방 안 시점(가운데·왼쪽 모서리·오른쪽 모서리 프리셋, 15° 회전·150mm 이동·화각 60–90°)은 공통 시점만 저장하고 장면·견적·편집 이력은 바꾸지 않는다. 방 안 시점은 눈높이 1,500mm·세로선이 곧은 구도이며 자재 정보가 없는 앞벽 쪽은 보지 않는다. 방 안에서만 천장과 평판 등이 보인다. D1 저장 실패 뒤에는 추가 회전에도 오류와 재시도 버튼을 유지한다. 사용자가 시점 저장을 재시도하면 최신 각도를 저장하며, 실패 중 서버에 있던 시점은 덮어쓰지 않는다.
...
일반 PNG/JPG 내보내기는 현재 시안이나 Before/After 비교를 렌더링하며 편집 도구를 이미지에 포함하지 않는다. 3D 렌더러를 쓰는 다운로드(공간 둘러보기, 벽 구조가 있는 프로젝트의 편집기 내보내기)는 여러 장을 겹쳐 부드러운 그림자와 윤곽을 만들고(기본 32장·약 10초 예산, 진행률 %와 취소, GPU 없는 기기는 가로 타일로 나눠 그려 화면이 멈추지 않고 장 수가 줄어듦), "사진 효과"(약한 빛 번짐·가장자리 어두움·입자감·톤 곡선, 기본 켬, 브라우저에 선택 기억)를 켜고 끌 수 있다. 시안 미리보기와 FLUX 입력은 한 장·효과 없음 그대로다. 근거: [렌더러 계약](../../../../docs/room-viewer-renderer-contract-20260914.md), [결과](../../../../docs/export-realism-stage-1-results-20260926.md). FLUX 변환은 현재 After PNG를 기준으로 사용자가 klein 4B를 실행하고 결과를 미리 본 뒤 PNG로 다운로드한다. 배치한 제품의 종류·위치·크기·색(enum·숫자만)을 함께 보내고, 서버가 그 정보로 프롬프트를 만든다. 제품 사진은 보내지 않는다(실측 비교에서 구도가 바뀜). 다시 만들기는 같은 기준 PNG·제품 정보에 새 seed를 쓴다. 결과는 내보내기 창에서 관리하며 프로젝트 장면을 자동 교체하지 않는다. 사진처럼 보여도 실제 현장 촬영본은 아니다.
...
| 렌더링         | [render](../../../../src/lib/render/), [room-viewer](../../../../src/lib/room-viewer/): 편집 미리보기·공간 모형·이미지 출력. 공간 둘러보기와 벽 구조가 있는 프로젝트의 편집·내보내기·시안 카드는 3D 렌더러, 그 밖의 기본 방·사진 프로젝트는 2D 합성을 쓴다. 3D 렌더러는 편집 화면·미리보기·내보내기 모두 천장 등의 설비 그림자를 같은 규칙([그림자](../../../../src/lib/room-viewer/shadow.ts))으로 그린다. 조명·재질 규칙은 [사실감 조명](../../../../src/lib/render/realistic-lighting.ts)·[마감](../../../../src/lib/render/finish.ts)·[렌더러 계약](../../../../docs/room-viewer-renderer-contract-20260914.md) 참고 |
